...
try:
    # Unified 4-value interface: (vectorizer=None), model_pipeline, folder_name_map, trained_labels, meta
    _, model, folder_name_map, trained_labels, meta = load_model_by_version(selected_version)
...
if file is not None:
    ...
    if st.button("Run Evaluation", key="diag_eval_run_btn"):
        preds = predict_batch_df(
            ...
            title_col=title_col,
            desc_col=desc_col,
            model=model,  # ← pipeline model only
